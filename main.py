...
def get_kj(owners, is_city):
    ...
    return city_kj.get(owners, .4) if is_city else vill_kj.get(owners, .3)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if choice[0] == "exit":
            ...
            if choice[3][0] == "e":
                pwr = int(choice[3][1:]) * 7.18
            elif choice[3][0] == "p":
                pwr = float(choice[3][1:])
            else:
                pwr = 0
            grid.add_node(int(choice[2]), pwr, int(choice[1]))
        elif choice[0] == "len":
